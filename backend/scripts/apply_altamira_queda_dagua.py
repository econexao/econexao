"""Apply Altamira Balneário e Pousada Queda D'água route and corridor associations (ECO-2628)."""
...
ROTA_QUEDA_DAGUA_ID = "a17a314a-0000-4000-8000-000000000005"


def calculate_queda_dagua_corridor(
    ...
    return {
        ...
        "route": "rota-queda-dagua",
        "route_id": ROTA_QUEDA_DAGUA_ID,
...
async def apply(dry_run: bool = False, db_url_override: str | None = None) -> int:
    ...
    logger.info("Computing Queda D'água corridor...")
    summary = calculate_queda_dagua_corridor(csv_path, geoms_path)
    logger.info(
        "Queda D'água corridor: %d unique actors "
    ...
    out_summary = DATA_DIR / "queda_dagua_ingestion_summary.json"
    ...
    migration_sql_path = (
        ROOT_DIR / "supabase" / "migrations" / "20260919110000_queda_dagua_four_origins_route.sql"
    )
