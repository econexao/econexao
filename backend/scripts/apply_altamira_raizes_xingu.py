"""Apply Altamira Sítio Raízes do Xingu route and corridor associations (ECO-2629)."""
...
ROTA_RAIZES_XINGU_ID = "a17a314a-0000-4000-8000-000000000006"


def calculate_raizes_xingu_corridor(
    ...
    return {
        ...
        "route": "rota-raizes-do-xingu",
        "route_id": ROTA_RAIZES_XINGU_ID,
...
async def apply(dry_run: bool = False, db_url_override: str | None = None) -> int:
    ...
    os.environ.setdefault("ROUTING_PROVIDER", "google_routes")
    settings = Settings()
    ...
    logger.info("Computing Sítio Raízes do Xingu corridor...")
    summary = calculate_raizes_xingu_corridor(csv_path, geoms_path)
    logger.info(
        "Sítio Raízes do Xingu corridor: %d unique actors "
    ...
    out_summary = DATA_DIR / "raizes_xingu_ingestion_summary.json"
    ...
    migration_sql_path = (
        ROOT_DIR / "supabase" / "migrations" / "20260919120000_raizes_xingu_four_origins_route.sql"
    )
