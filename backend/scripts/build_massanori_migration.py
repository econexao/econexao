...
GEOMS_FILE = ROOT / "docs" / "data" / "altamira" / "massanori_geometries.json"
OUT_SQL = ROOT / "supabase" / "migrations" / "20260918193000_massanori_four_origins_route.sql"

with open(GEOMS_FILE, encoding="utf-8") as f:
    geoms = json.load(f)

summary_txt = (
    "Famosa praia de água doce às margens do Rio Xingu em Altamira, "
    "a Praia do Massanori se destaca pela extensa faixa de areia dourada durante o período "
    "de vazante, quiosques com gastronomia regional e infraestrutura de lazer."
)
season_txt = (
    "Período de seca e estiagem do Rio Xingu (julho a janeiro), "
    "quando surgem as praias fluviais de areia clara e águas calmas."
)
conn_txt = (
    "Cobertura 4G/3G no trajeto urbano e oscilação no trecho final de acesso à praia."
)
access_txt = (
    "Acesso viário pavimentado no perímetro urbano de Altamira e estrada vicinal/piçarra "
    "até o bolsão de estacionamento e orla da praia."
)
payment_txt = (
    "Pix e dinheiro em espécie são amplamente aceitos; cartões de débito/crédito funcionam "
    "na maioria dos quiosques conforme sinal."
)

sql_lines = [
    "-- Migration: 20260918193000_massanori_four_origins_route.sql",
    "-- Description: Add official route Praia do Massanori in Altamira with 4 OSRM origins.",
    "",
    "BEGIN;",
    "",
    "-- 1. Insert or update Rota Praia do Massanori",
    ...
    "    updated_at",
    ") VALUES (",
    "    'a17a314a-0000-4000-8000-000000000003',",
    ...
    "",
    "-- 2. Insert 4 Official Origins for Praia do Massanori",
    ...
    ") VALUES",
    "    (",
    "        'a17a314a-0000-4000-8000-000000000021',",
    "        'a17a314a-0000-4000-8000-000000000003',",
    "        'rodoviaria',",
    ...
    "    ),",
    "    (",
    "        'a17a314a-0000-4000-8000-000000000022',",
    "        'a17a314a-0000-4000-8000-000000000003',",
    "        'aeroporto',",
    ...
    "    ),",
    "    (",
    "        'a17a314a-0000-4000-8000-000000000023',",
    "        'a17a314a-0000-4000-8000-000000000003',",
    "        'terminal_fluvial',",
    ...
    "    ),",
    "    (",
    "        'a17a314a-0000-4000-8000-000000000024',",
    "        'a17a314a-0000-4000-8000-000000000003',",
    "        'centro',",
...
origin_uuids = {
    "rodoviaria": "a17a314a-0000-4000-8000-000000000021",
    "aeroporto": "a17a314a-0000-4000-8000-000000000022",
    "terminal_fluvial": "a17a314a-0000-4000-8000-000000000023",
    "centro": "a17a314a-0000-4000-8000-000000000024",
}
...
sql_lines.extend([
    ...
    "    WHERE ro.route_id = 'a17a314a-0000-4000-8000-000000000003'",
    ...
    "WHERE ra.route_id = 'a17a314a-0000-4000-8000-000000000003'",
    "  AND NOT EXISTS (",
    "      SELECT 1",
    "      FROM app_private.actors a",
    "      CROSS JOIN app_private.route_origins ro",
    ...
    "        AND ro.route_id = 'a17a314a-0000-4000-8000-000000000003'",
