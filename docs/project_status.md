...
- **Evidência adicional:** primeira chamada ao health canônico excedeu 40s; segunda tentativa de health/regiões/preflight ficou aproximadamente entre 0,3–0,4s e a UI recuperou. A causa não foi determinada.
- **Referência:** [direcionamento_versao_web_evento.md](direcionamento_versao_web_evento.md). **Commit:** Não vinculado.
...
- **Evidência local reproduzida nesta branch:** `getAuthIdentity` classifica visitante, guest anônimo inclusive com e-mail transitório e conta; leituras de favoritos propagam `AbortSignal`; a troca A→B aborta consulta privada, remove seu cache e preserva regiões públicas; Jest passou 44 suítes/282 testes; typecheck e OpenAPI passaram; Playwright fixture passou 6/6 em Chromium desktop/mobile para salvar/remover rota e ator, rollback acessível, restauração, isolamento A/B, paginação sem duplicação e descarte de resposta obsoleta; export Web passou. A fixture Playwright é local e não usa Supabase/Google reais. Homologação autenticada no canônico não foi executada; não há evidência remota de favoritos funcionais.
- **Referência:** [finalization/tasks.md](finalization/tasks.md). **Commit:** Não vinculado.
