...
- **Evidência local reproduzida nesta branch:** `getAuthIdentity` classifica visitante, guest anônimo inclusive com e-mail transitório e conta; leituras de favoritos propagam `AbortSignal`; a troca A→B aborta consulta privada, remove seu cache e preserva regiões públicas. O foco Jest passou 4 suítes/31 testes; typecheck e OpenAPI passaram. `npm run export:web:fixture` executou `expo export -p web --clear` com URL/API, URL Supabase e publishable key públicas de fixture; o Playwright abriu o bundle real em baseURL e passou 8/8 em Chromium desktop/mobile, cobrindo detalhe de ator, catálogo de rota, rollback, reload, tab de rotas, paginação de atores sem duplicação e favoritos de rota. As fixtures não são Supabase/API reais; visitante/guest/conta reais, isolamento A/B real, renovação de sessão e persistência no staging continuam pendentes.
- **Referência:** [finalization/tasks.md](finalization/tasks.md). **Commit:** Não vinculado.
