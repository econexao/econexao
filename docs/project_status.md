...
#### ECO-2626 — Preparar, importar e verificar rota: Praia do Massanori (Altamira/PA)

- **Estado / horizonte / alteração:** CONCLUÍDA LOCAL / Versão do evento / NOVA.
- **Dependências ou sucessoras:** ECO-2605, ECO-2611, ECO-2621, ECO-2622.
- **Conclusão / aceite:** Ficha, 4 origens oficiais (Rodoviária, Aeroporto, Terminal Fluvial, Centro), geometrias OSRM contínuas até o acesso viário (-3.209977, -52.140359), metadados editoriais, imagem oficial e associação de 435 atores únicos no corredor de 1.000 m (280 rodoviária, 145 aeroporto, 168 terminal fluvial, 173 centro) preparadas e validadas; dry-run aprovado, testes unitários frontend (58 suítes / 366 testes), testes backend (39 testes), typecheck e export Web concluídos com exit code 0; migration versionada 20260918193000 pronta para carga remota mediante GO.
...
#### ECO-2628 — Preparar, importar e verificar rota: Balneário e Pousada Queda D'água (Altamira/PA)

- **Estado / horizonte / alteração:** CONCLUÍDA LOCAL / Versão do evento / NOVA.
- **Dependências ou sucessoras:** ECO-2605, ECO-2611, ECO-2621, ECO-2622, ECO-2626, ECO-2627.
- **Conclusão / aceite:** Ficha, 4 origens oficiais (Rodoviária, Aeroporto, Terminal Fluvial, Centro), geometrias OSRM contínuas pela BR-230 até o entroncamento viário (-3.280970, -52.385373), metadados editoriais e associação de 398 atores únicos no corredor de 1.000 m (250 rodoviária, 74 aeroporto, 357 terminal fluvial, 356 centro) preparadas e validadas; dry-run aprovado, testes unitários frontend, testes backend, typecheck e export Web concluídos com exit code 0; migration versionada 20260919110000 pronta para carga remota mediante GO.
- **Evidência e limite:** Inserção preparada em 19/09/2026 na branch codex/eco-2628-rota-queda-dagua sobre o SHA b34c6ac; queda_dagua_geometries.json, queda_dagua_route_package.md e queda_dagua_report.md criados; dry-run comprovado em queda_dagua_ingestion_summary.json. Nenhuma escrita no banco remoto ou push de branch executados antes dos respectivos GOs.
- **Referência:** [docs/data/altamira/queda_dagua_report.md](data/altamira/queda_dagua_report.md). **Commit:** Não vinculado.

#### ECO-2629 — Preparar, importar e verificar rota: Sítio Raízes do Xingu (Altamira/PA)

- **Estado / horizonte / alteração:** CONCLUÍDA LOCAL / Versão do evento / NOVA.
- **Dependências ou sucessoras:** ECO-2605, ECO-2611, ECO-2621, ECO-2622, ECO-2626, ECO-2627, ECO-2628.
- **Conclusão / aceite:** Ficha, 4 origens oficiais (Rodoviária, Aeroporto, Terminal Fluvial, Centro), geometrias OSRM contínuas pela BR-230 até o entroncamento viário (-3.353302, -52.561737), metadados editoriais, atrativo Cachoeira Planaltina em destaque, galeria de 4 fotos oficiais e associação de 398 atores únicos no corredor de 1.000 m (250 rodoviária, 74 aeroporto, 357 terminal fluvial, 356 centro) preparadas e validadas; dry-run aprovado, testes unitários frontend, testes backend, typecheck e export Web concluídos com exit code 0; migration versionada 20260919120000 pronta para carga remota mediante GO.
- **Evidência e limite:** Inserção preparada em 19/09/2026 na branch codex/eco-2629-rota-raizes-do-xingu sobre o SHA 66b3498; raizes_xingu_geometries.json, raizes_xingu_route_package.md e raizes_xingu_report.md criados; dry-run comprovado em raizes_xingu_ingestion_summary.json. Nenhuma escrita no banco remoto ou push de branch executados antes dos respectivos GOs.
- **Referência:** [docs/data/altamira/raizes_xingu_report.md](data/altamira/raizes_xingu_report.md). **Commit:** Não vinculado.
...
- **Evidência e limite:** Implementação local no worktree isolado; typecheck, OpenAPI, Jest completo, export fixture e E2E normal/reduced motion/histórico (8/8 desktop/mobile) passaram. Sem vídeo/trace dedicado, homologação nativa ou publicação remota.
- **Referência:** [Evidência ECO-2705](motion_design/evidence/ECO-2705.md), [Plano](motion_design/implementation_plan.md), [prompt](motion_design/prompts/ECO-2705.md), [protocolo](motion_design/execution_protocol.md). **Commit:** Local na branch `codex/eco-2705-motion`.
...
- **Evidência e limite:** Implementação local no worktree isolado; typecheck, OpenAPI, Jest completo (57/57), export fixture e browser de modal/focus (2/2 desktop/mobile) passaram. Sem homologação nativa, vídeo/trace dedicado ou publicação remota.
- **Referência:** [Evidência ECO-2706](motion_design/evidence/ECO-2706.md), [Plano](motion_design/implementation_plan.md), [prompt](motion_design/prompts/ECO-2706.md), [protocolo](motion_design/execution_protocol.md). **Commit:** Local na branch `codex/eco-2706-motion`.
...
- **Conclusão / aceite:** candidato publicável reconciliado e preparado com manifesto release-candidate.md (diff de 61 arquivos), evidências locais completas (Typecheck, OpenAPI, Jest 57/354, Export Web, Playwright 40/40 com asserções explícitas de console limpo), parecer de revisão independente formalizado em ECO-2708-review.md (APPROVE), PR #70 aberta com checks verdes e plano de rollback/homologação.
- **Evidência e limite:** Manifesto de release candidate, evidência ECO-2708 e relatório de revisão independente versionados. Branch de integração publicada na PR #70 com checks remotos aprovados. Proteção de `staging` verificada com atualização estrita da base e checks obrigatórios `Validate branch promotion flow`, `contract` e `Vercel`, inclusive para administradores; force push e exclusão bloqueados. Sem merge ou publicação em staging nesta task.
- **Referência:** [Evidência ECO-2708](motion_design/evidence/ECO-2708.md), [Revisão Independente](motion_design/evidence/ECO-2708-review.md), [Release Candidate](motion_design/evidence/release-candidate.md), [PR #70](https://github.com/econexao/econexao/pull/70), [Plano](motion_design/implementation_plan.md), [prompt](motion_design/prompts/ECO-2708.md), [protocolo](motion_design/execution_protocol.md). **Commit:** Branch `codex/motion-integration` publicada na PR #70.
...
- **Refinamento visual — 19/09/2026:** oito slides, incluindo Altamira e região; fotos derivadas exclusivamente das galerias do app, quatro capturas atuais fornecidas pelo owner e Home `home-app.PNG`. Branch `codex/landing-play-visual`, worktree `eco-nexao-play-redesign`, base staging `4845028`. Validação local: suíte `node landing-page/tests/test-landing-page.mjs` passou (landing/newsletter mock, navegação, swipe, QR, refresh, imagens, aliases com CSS, reduced motion e 32 combinações slide/viewport em 1440/1024/390/320 px); inspeção visual desktop/mobile e revisão independente realizadas. Ajustados caminhos absolutos para URLs com barra e legenda Brasil Novo. Projeto remoto confirmado read-only: `econexao-landing-production`, sem vínculo Git, deploy atual por CLI; push staging aciona pipeline do app e não publica a landing. GO do owner recebido em 19/09/2026 para commit, integração/push em staging e deploy manual da landing; confirmação remota pendente neste registro. Evidências e proveniência em `landing-page/README.md`; nenhum código/configuração do aplicativo alterado.
- **Revisão editorial do owner:** fontes e telas ampliadas, localidades do slide 5 corrigidas, slide 6 dedicado a Brasil Novo/Região Xingu (Altamira), narrativa de mapeamento de rotas e convite aos empreendedores para cadastro. Slide 8: “Sua próxima parada é no ECOnexão”, contato por e-mail e QR maior. Revalidação local da suíte e das 32 combinações passou; sem publicação.
- **Segunda revisão editorial:** URL escrita no slide 1; sugestões de destinos via #econexaoturismo no slide 6; imagens produto-app/servico-app do owner, e-mail na caixa amarela e cadastro compacto no slide 7. Testes locais desktop/mobile repetidos; nenhuma publicação.
