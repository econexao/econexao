...
- **Agora:** Barra de abas inferior (`app/(tabs)/_layout.tsx`) atualizada com suporte dinâmico a safe area via `useSafeAreaInsets` (`height: 64 + insets.bottom`, `paddingBottom: Math.max(insets.bottom, 8)`), eliminando o corte de ícones e rótulos ("Inicial", "Rotas", "Perfil") em dispositivos Android com gesture navigation, iOS e navegadores móveis. Suíte com 47 arquivos e 300 testes Jest passando 100%, typecheck e export Web verificados localmente. Publicação organizada em staging (`https://econexao-app-staging.vercel.app/`).
