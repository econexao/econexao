...
async function mockContract(page: import('@playwright/test').Page, photoMode: 'ready' | 'empty' | 'metadata-error' | 'image-error' = 'ready') {
  let photoRequests = 0;
  let metadataRequests = 0;
  await page.route('**/*', async (route) => {
    const url = new URL(route.request().url());
    if (url.protocol === 'https:' && !url.hostname.endsWith('example.test') && !url.hostname.includes('google.com') && !url.hostname.includes('supabase.co')) {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.endsWith('/google-photo')) {
      ...
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: {
        proxy_url: photoMode === 'image-error' ? 'https://fixture.invalid/photo.jpg' : 'http://127.0.0.1:8082/fixture-photo.jpg',
  ...
  await page.route('**/fixture-photo.jpg', (route) => route.fulfill({ status: 200, contentType: 'image/jpeg', body: FIXTURE_JPEG }));
  return { get photoRequests() { return photoRequests; }, get metadataRequests() { return metadataRequests; } };
}
...
test.describe('ECO-2613 fotos e perfil', () => {
  test('cards usam editorial/placeholder e não consultam fotos Google', async ({ page }) => {
    ...
    await expect(page.getByText('Ateliê Tapajós').first()).toBeVisible();
  ...
  for (const mode of ['empty', 'metadata-error', 'image-error'] as const) {
    test(`detalhe trata ${mode} honestamente e oferece retry`, async ({ page }) => {
      const counters = await mockContract(page, mode);
      await page.goto('/actor/actor-no-cover');
      await expect(page.getByText(mode === 'empty' ? 'Foto indisponível.' : 'Não foi possível exibir esta foto.')).toBeVisible({ timeout: 15000 });
